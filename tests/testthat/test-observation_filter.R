...
test_that("observation_filter works", {
  ...
  cleaned_outbreak_ts<-clean_psql_data(original_data=outbreak_ts_tmp)
  lower_time_bound<-lubridate::ymd("2014-03-24")
  upper_time_bound<-lubridate::ymd("2014-05-18")
  filtered_outbreak_ts<-observation_filter(outbreak_data=cleaned_outbreak_ts,
...
test_that("observation_filter works", {
  ...
  outbreak_ts_tmp<-outbreak_ts%>%subset(location_period_id==1123)
  cleaned_outbreak_ts<-clean_psql_data(original_data=outbreak_ts_tmp)
  cleaned_outbreak_ts[1:3,]$temporal_scale<-"daily"
  filtered_outbreak_ts<-observation_filter(outbreak_data=cleaned_outbreak_ts,
...
test_that("observation_filter works", {
  ...
  outbreak_ts_tmp<-outbreak_ts%>%subset(location_period_id==1123)
  cleaned_outbreak_ts<-clean_psql_data(original_data=outbreak_ts_tmp)
  cleaned_outbreak_ts[1:3,]$who_region<-"SEAR"
  filtered_outbreak_ts<-observation_filter(outbreak_data=cleaned_outbreak_ts,
                                           who_regions=c("AFR")
  )
...
test_that("observation_filter works", {
  ...
  outbreak_ts_tmp<-outbreak_ts%>%subset(location_period_id==1123)
  cleaned_outbreak_ts<-clean_psql_data(original_data=outbreak_ts_tmp)
  cleaned_outbreak_ts[1:3,]$spatial_scale<-c("admin3","admin1","admin2")
  filtered_outbreak_ts<-observation_filter(outbreak_data=cleaned_outbreak_ts,
                                           spatial_scale_filter = "country")
  testthat::expect_true(all(filtered_outbreak_ts$spatial_scale=="country"))
})

#test whether primary filter works
test_that("observation_filter works", {
  outbreak_ts<-readRDS("data/outbreak_data.rds")
  outbreak_ts_tmp<-outbreak_ts%>%subset(location_period_id==1123)
  cleaned_outbreak_ts<-clean_psql_data(original_data=outbreak_ts_tmp)
  cleaned_outbreak_ts[1:3,]$primary<-"f"
  filtered_outbreak_ts<-observation_filter(outbreak_data=cleaned_outbreak_ts)
  testthat::expect_true(all(filtered_outbreak_ts$primary=="t"))
})
...
test_that("observation_filter works", {
  ...
  outbreak_ts_tmp<-outbreak_ts%>%subset(location_period_id==1123)
  cleaned_outbreak_ts<-clean_psql_data(original_data=outbreak_ts_tmp)
  cleaned_outbreak_ts[1:3,]$sCh<-NA
  filtered_outbreak_ts<-observation_filter(outbreak_data=cleaned_outbreak_ts,
                                           remove_na_sCh=FALSE)
...
test_that("observation_filter works", {
  ...
  outbreak_ts_tmp<-outbreak_ts%>%subset(location_period_id==1123)
  cleaned_outbreak_ts<-clean_psql_data(original_data=outbreak_ts_tmp)
  cleaned_outbreak_ts[1:3,]$cCh<-NA
  filtered_outbreak_ts<-observation_filter(outbreak_data=cleaned_outbreak_ts,
                                           remove_na_cCh=TRUE)
