...
# Loads every stage2_{who_region}_{country_iso3}.parquet file in generated_data/,
# binds rows, and saves a combined parquet + CSV.
#
...
option_list <- list(
  make_option(c("-w", "--workers"),
              ...
              help    = "Number of parallel workers for loading parquet files [default: 8]"),
  ...
  make_option(c("-o", "--out_dir"),
              ...
              help    = "Directory containing stage2_*.parquet files")
)
...
stage2_files <- list.files(opt$out_dir,
                            pattern    = "^stage2_.*\\.parquet$",
                            full.names = TRUE)

if (length(stage2_files) == 0) {
  stop("No stage2_*.parquet files found in: ", opt$out_dir,
